...
| Publish or retain immutable skill snapshots | `resolve_skill_runtime_bundles`, `publish_skill_snapshots`, `close_skill_snapshot_references` | Owner-resolved complete bundles, content-addressed snapshots, and stable active/closed project references. |
...
| --- | --- | --- | --- |
| `run_subagent` | `run_kind: "quick_noninteractive"` | `continuity`, `effect_profile`, `expected_skill_sha256`, `skill_snapshot_binding`, `recursive_delegation` | `timeout_ms`, top-level `session_id` |
| `schedule_run` | none | `continuity`, `timeout_ms`, `wait_ms`, `effect_profile`, `expected_skill_sha256`, `skill_snapshot_binding`, `recursive_delegation` | top-level `session_id` |
| `start_run` | none | `continuity`, `timeout_ms`, `effect_profile`, `expected_skill_sha256`, `skill_snapshot_binding`, `recursive_delegation` | top-level `session_id` |
| `start_session_run` | `session_key` | `resume_mode`, `packet_policy`, `timeout_ms` | `continuity`, top-level `session_id`, `effect_profile`, `expected_skill_sha256`, `skill_snapshot_binding`, `recursive_delegation` |
| `run_subagent_session` | `session_key` | `resume_mode`, `packet_policy`, `timeout_ms` | `continuity`, top-level `session_id`, `effect_profile`, `expected_skill_sha256`, `skill_snapshot_binding`, `recursive_delegation` |
...
Snapshot bindings support `run_subagent`, `start_run`, and `schedule_run` with ephemeral, fresh, and raw-resume continuity; raw resume still requires recursive-delegation reauthorization. Named sessions reject them. Enabled descendants inherit only the confirmed ancestor snapshot binding and cannot widen to another skill. Both effect profiles exclude delegation. This is owner filesystem integrity plus Pi pre-prompt enforcement, not an OS sandbox or hostile-runtime claim.
...
- `get_run_contract` remains durable-run contract version `2` and advertises additive v1 skill operations plus recursive authority. New callers require `exact_root_runtime_bundle_validation`, `full_runtime_bundle_resolution`, `immutable_skill_snapshots`, `reference_retained_skill_snapshots`, `snapshot_bound_launch`, `explicit_skill_snapshot_deletion`, `explicit_recursive_delegation`, and `terminal_recursive_subtree_closure` as applicable.

...
For `effect_profile:"workspace_read_only"`, `src/piChild.ts` disables ambient extension discovery, explicitly binds `pi-search-hub`, passes the exact seven-tool allowlist to Pi at `createAgentSession`, excludes recursive control/delegate, verifies active tools, and emits a pre-prompt receipt. `effect_profile:"skill_creator_authoring_v1"` likewise disables ambient extensions and delegates only its six-tool list to Pi; `src/taskRootAuthoringTools.ts` overrides each filesystem tool to bound read/list/find/grep requests and reject lexical or resolved escapes. Writes/edits admit only the exact real run cwd; read/list/find/grep additionally admit the exact real complete runtime-bundle root only after a snapshot binding has revalidated it. Neither profile offers general shell or recursion. `src/runSubagent.ts` validates receipts before public/durable projection. When `expected_skill_sha256` is present, the parent verifies the canonical skill before launch and places those bytes in the run-owned child-request directory; Pi expands the snapshot while the receipt identifies the canonical source.

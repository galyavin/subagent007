...
## Loop 87 - Inline Single-Use Auto-Promotion Event Helper

Finding: `appendAutoPromotedEvent()` in `src/runTask.ts` is a one-call helper used only by `runSubagentPromotedTask()` immediately after building and attaching the promotion metadata. It no longer removes duplication, and the promoted-run setup is easier to verify when the promotion metadata and matching public event are adjacent.

Behavior check: inlining the same `[auto_promoted] run_subagent -> durable_run` task event, metadata copy, timestamp, and `run_subagent auto-promoted to durable run` progress message should not change observable behavior if the event remains written before the first promoted-run snapshot.

Oracle: existing run-subagent tests assert auto-promotion metadata and the raw `[auto_promoted] run_subagent -> durable_run` event for promoted one-shot requests. No new pinning test is needed for a same-event inline.

Decision: patch minimally. If any test fails, revert this loop and do not retry it.

Patch: removed `appendAutoPromotedEvent()` and inlined the same task event and progress message directly after promoted-run registration.

Targeted oracle result: `npm run typecheck`, `npm run build`, and `node scripts/run-tests-with-ledger-guard.mjs tests/run-subagent.test.ts` passed; targeted tests passed 43/43.

Full oracle result: `npm test` passed 136/136.

## Loop 88 - Inline Single-Use Config String Guard

Finding: `nonEmptyString()` in `src/config.ts` is a one-call helper used only by `modelClass()` to validate `default_model_class`. It no longer removes duplication and forces a jump away from the only field that uses the strict nonempty-string error contract.

Behavior check: inlining the same `undefined` allowance, string/nonempty check, trim, and `${key} must be a nonempty string when provided` error into `modelClass()` should not change observable behavior if canonical config trimming and invalid config rejection remain unchanged.

Oracle: existing validation tests cover config path resolution, raw versus normalized config records, canonical trimming, legacy migration, unsupported model-class rejection, and missing config behavior. No new pinning test is needed for a same-branch inline that preserves the exact message.

Decision: patch minimally. If any test fails, revert this loop and do not retry it.

Patch: removed `nonEmptyString()` and moved its exact `undefined`, nonempty-string, trim, and error-message behavior into `modelClass()`.
...
## Loop 90 - Inline Single-Use Legacy Thinking Parser

Finding: `legacyThinkingLevel()` in `src/config.ts` is a one-call helper used only by `normalizeConfigRecord()` for the legacy `default_thinking_level` migration path. It no longer removes duplication, and keeping the legacy model/thinking extraction together makes the migration branch easier to audit.

Behavior check: inlining the same `legacyNonEmptyString()` call plus `THINKING_LEVELS.includes(...)` check should not change observable behavior if malformed legacy values still silently produce no default model class and supported legacy pairs still migrate.

Oracle: existing validation and config-migrate tests cover legacy default model/thinking migration, malformed legacy config no-op behavior, unsupported legacy pairs, and canonical model-class precedence. No new pinning test is needed for a same-expression inline.

Decision: patch minimally. If any test fails, revert this loop and do not retry it.

Patch: removed `legacyThinkingLevel()` and inlined its exact legacy string extraction plus `THINKING_LEVELS` membership check into `normalizeConfigRecord()`.

Targeted oracle result: `npm run typecheck`, `npm run build`, and `node scripts/run-tests-with-ledger-guard.mjs tests/validation.test.ts tests/config-migrate.test.ts` passed; targeted tests passed 30/30.
...
## Loop 92 - Inline Single-Use Failure Logging Disable Check

Finding: `failureLoggingDisabled()` in `src/failureLog.ts` is a one-call helper that only checks `process.env.SUBAGENT007_FAILURE_LOG === "off"` inside `logFailure()`. It adds no reuse and no additional policy beyond the environment comparison itself.

Behavior check: inlining the same environment comparison should not change observable behavior if `SUBAGENT007_FAILURE_LOG=off` still suppresses failure-log writes and all other values leave logging enabled.

Oracle: existing failure-log tests cover disabled logging, normal failure writes, and schema/handler validation logging. No new pinning test is needed for a same-expression inline.

Decision: patch minimally. If any test fails, revert this loop and do not retry it.

Patch: removed `failureLoggingDisabled()` and checked `process.env.SUBAGENT007_FAILURE_LOG === "off"` directly in `logFailure()`.
...
## Loop 94 - Shared Test Skill Audit Hash Helper

Finding: the recent skill-audit tests duplicated the same `sha256File()` helper in `tests/run-subagent.test.ts` and `tests/session.test.ts`. This is test-only duplication introduced after the Loop 93 closure scan; both files need the same hash projection to assert `resolved_skill_sha256`.

Behavior check: moving the helper into `tests/helpers/testUtils.ts` should not change externally observable server behavior because it touches only test code. It must preserve the exact SHA-256 calculation over raw file bytes used by the existing assertions.

Oracle: existing run-subagent and session tests exercise every use of the helper: direct skill runs, normalized `skill_name`, auto-promoted skill runs, `start_run`, and named-session run records. No new pinning test is needed for a helper relocation that keeps the same assertions.

Decision: patch minimally. If any test fails, revert this loop and do not retry it.

Patch: exported `sha256File()` from `tests/helpers/testUtils.ts`, imported it from the two affected test files, and removed the local duplicate helpers and unused `createHash` imports.

Targeted oracle result: `npm run typecheck`, `git diff --check`, and `npm run build && node scripts/run-tests-with-ledger-guard.mjs tests/run-subagent.test.ts tests/session.test.ts` passed; targeted tests passed 58/58.
